    ...
    · apply mul_ne_zero
      · rw [mul_pow (χ_of_v_of_t) (v_of_t) ((q + 1) / 4)]
        ...
        · apply pow_ne_zero (((q + 1) / 4) : ℕ)
          apply LegendreSymbol.χ_a_ne_zero v_ne_zero q_h1
        · apply pow_ne_zero (((q + 1) / 4) : ℕ)
          apply v_ne_zero
      · apply LegendreSymbol.χ_a_ne_zero v_ne_zero q_h1
    · apply LegendreSymbol.χ_a_ne_zero (v_h1_third_factor_ne_zero s_h1 q_h1 q_h2 q_h3 t) q_h1
...
lemma one_add_X_ne_zero
  ...
  (1 + X_of_t) ≠ (0 : F) := by
    ...
    have h3 : v_of_t = -χ_of_v_of_t * (1 + r_of_s^2 - 2 + 1) := by
      ...
      rw [LegendreSymbol.χ_of_a_pow_n_eq_χ_a v_of_t ⟨5, h3_1⟩ q_h1 q_h2 q_h3]
      have h3_2: Odd 3 := by
        apply Nat.odd_iff.2
        norm_num
    ...
    have h5 : χ_of_v_of_t = -χ_of_v_of_t := by
      ...
      rw [LegendreSymbol.χ_of_a_mul_b_eq_χ_of_a_mul_χ_of_b] at h1
      rw [LegendreSymbol.χ_of_neg_one_eq_neg_one q_h1 q_h2 q_h3] at h1
      rw [LegendreSymbol.χ_of_χ_of_a_eq_χ_of_a q_h1 q_h2 q_h3] at h1
      change -1 * χ_of_v_of_t * LegendreSymbol.χ (r_of_s ^ 2) * -χ_of_v_of_t = -1 at h1
      have h5_1 : r_of_s^2 ≠ 0 := by
        rw [pow_two]
        apply mul_ne_zero
        · apply r_ne_zero s_h1 q_h1 q_h2 q_h3
        · apply r_ne_zero s_h1 q_h1 q_h2 q_h3
      have h5_2 : IsSquare (r_of_s^2) := by
        rw [pow_two]
        apply IsSquare.mul_self r_of_s
      rw [LegendreSymbol.χ_a_eq_one h5_1 h5_2 q_h1 q_h3] at h1
      rw [mul_one] at h1
      ...
      rw [mul_one] at h1
      exact h1
    have h6 : χ_of_v_of_t ≠ -χ_of_v_of_t := LegendreSymbol.neg_χ_a_ne_χ_a v_ne_zero q_h1 q_h2 q_h3
    contradiction
    ...
    have first_factor : (χ_of_v2 * v2)^((q + 1) / 4) = (χ_of_v1 * v1)^((q + 1) / 4) * χ_of_u1 / u1^3:= by
      ...
      have h1_2 : IsSquare (χ_of_u1 * u1^3) := by
        have h1_2_1 : χ_of_u1 * u1^3 ≠ 0 := by
          apply mul_ne_zero
          · apply LegendreSymbol.χ_a_ne_zero u_ne_zero q_h1
          · apply pow_ne_zero 3 u_ne_zero
        apply (LegendreSymbol.χ_a_eq_one_iff_a_square h1_2_1 q_h1 q_h3).mp
      ...
      have h1_3 : (u1^6)^((q + 1) / 4) = χ_of_u1 * u1^3  := by
        ...
        rw [mul_pow, LegendreSymbol.χ_of_a_pow_n_eq_χ_a u1 ⟨3, h1_3_2 ⟩ q_h1 q_h2 q_h3]
      calc
      ...
      calc
        ...
        _ = LegendreSymbol.χ ((u1^2 * (c_of_s^2 + u1^2)) * (u1^2 + 1 / c_of_s^2)^2) := by
          ...
          have h1 : c_of_s ^ 2 * u1 ^ 4 * (1 / u1 * (1 / u1) + 1 / c_of_s ^ 2) = u1^2 * (c_of_s^2 + u1^2) := by
            ...
            rw [mul_inv_cancel₀ (c_pow_two_ne_zero s_h1 q_h1 q_h2 q_h3)]
    ...
    calc
      ...
      _ = Y1 / (χ_of_v1 * u1)^3 := by
        ...
        have h2 : Odd 3 := by
          ...
          norm_num
        let v_ne_zero := v_ne_zero s_h1 q_h1 q_h2 q_h3 t
        rw [← LegendreSymbol.χ_of_a_pow_n_eq_χ_a v1 ⟨3, h2⟩ q_h1 q_h2 q_h3]
