...
lemma t2_eq_one
  ...
  t2_of_P = 1 := by
    ...
    unfold u2_of_P
    rw [u2_eq_zero t hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod]
    simp
...
lemma t2_eq_t'
  ...
  t2_of_P = t' := by
    ...
    let u' := u ⟨t', t_h⟩
    let u2 := u2 s P q
    let h : u2 = u' := u2_eq_u' t hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod X_h
    unfold u' u at h
    unfold t2_of_P t2
    change (1 - u2) / (1 + u2) = t'
...
lemma t2_in_t_or_neg_t
  ...
  t2_of_P = t ∨ t2_of_P = t' := by
    intro P t' t2_of_P
    by_cases h : t ≠ 1 ∧ t ≠ -1
    · rcases (X2_h4 ⟨t, h⟩ hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod) with h1 | h1
      · left
        exact t2_eq_t ⟨t, h⟩ hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod h1
      · right
        exact t2_eq_t' ⟨t, h⟩ hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod h1
    · have h' : t = 1 ∨ t = -1 := by
        ...
        exact h
      unfold t2_of_P t'
      rw [t2_eq_one ⟨t, h'⟩ hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod]
      grind
...
lemma u'_eq_u
  ...
  let t_h := t'_ne_one_and_t'_ne_neg_one_of_X2_ne_one
    hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod P P_props x_ne_zero y_ne_one X_h
  let u := u ⟨t, t_h⟩
...
lemma X'_eq_X
  ...
  X' = X := by
    ...
    let h2 := u'_eq_u hs_ne_zero
      sq_ne_pm_two hq_card hq_primePow hq_mod P P_props x_ne_zero y_ne_one X_h
...
theorem x'_and_y'_fulfill_curve_equation
  ...
  edwardsCurveEquation x' y' ⟨d, d_h⟩ := by
    ...
    let y'_eq_y := y'_eq_y hs_ne_zero
      sq_ne_pm_two hq_card hq_primePow hq_mod P P_props x_ne_zero y_ne_one X_h
    let h := curve_equation ⟨t, t_h⟩ hs_ne_zero sq_ne_pm_two hq_card hq_primePow hq_mod
...
lemma y_eq_y_of_P
  ...
  y = y_of_P := by
    ...
    let h := X'_eq_X hs_ne_zero sq_ne_pm_two
      hq_card hq_primePow hq_mod P P_props x_ne_zero y_ne_one X_h
    grind
