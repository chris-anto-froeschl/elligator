      ...
      calc
        ...
        _ = (χ_of_v * v)^((q + 1) / 2) * 1 := by
          ...
          rw [χ_of_a_even_pow_n_eq_one
            (v_h1_third_factor_ne_zero s_h1 q_h1 q_h3 t) ⟨2, even_two⟩ q_h1 q_h3]
          rw [mul_one]
...
lemma y_add_one_ne_zero
  ...
  y + 1 ≠ (0 : F) := by
    ...
    have h4 : r * X = 0 := by
      ...
      exact h3
    have h5 : r * X ≠ 0 := by
      apply mul_ne_zero (r_ne_zero s_h1 q_h1 q_h3) (X_ne_zero s_h1 q_h1 q_h3 t)
    contradiction
...
lemma curve_equation
  ...
  x^2 + y^2 = 1 + d * x^2 * y^2 := by
    ...
    have h2 : Y^2 * (1 - x^2) = X * (r * X - (1 + X)^2)^2 := by
      calc
        Y^2 * (1 - x^2) = Y^2 - (c - 1)^2 * s^2 * X^2 * (1 + X)^2 := by
          ...
          rw [mul_sub, mul_one]
          have h2_1 : Y^2 ≠ 0 := pow_two_ne_zero (Y_ne_zero s_h1 q_h1 q_h3 t)
          grind
    ...
    have h4 : -d * (c - 1)^2 * s^2 = 2 * (r + 2) := by
      rw [neg_d_eq_r_add_two_over_r_sub_two s_h1 q_h1 q_h3, mul_assoc, h1]
    ...
    have h7 : Y^2 * (1 - d * x^2) ≠ 0 := by
      apply mul_ne_zero
      · exact pow_two_ne_zero (Y_ne_zero s_h1 q_h1 q_h3 t)
      · exact h6
    have h8 : (1 - x^2) / (1 - d * x^2) = y^2 := by
      calc
        (1 - x^2) / (1 - d * x^2) = (r * X - (1 + X)^2)^2 / (r * X + (1 + X)^2)^2 := by
          have h8_1 : Y^2 / Y^2 = 1 := by
            have h7_2 : Y^2 ≠ 0 := pow_two_ne_zero (Y_ne_zero s_h1 q_h1 q_h3 t)
            rw [div_self h7_2]
          nth_rw 1 [← one_mul (1 - x^2), ← h8_1]
          rw [mul_div_assoc, ← mul_div_mul_comm, h2, h5]
          rw [mul_div_mul_comm X _ X _, div_self (X_ne_zero s_h1 q_h1 q_h3 t), one_mul]
        _ = y^2 := by
          rw [← div_pow _ _ 2]
          change y^2 = y^2
          rfl
