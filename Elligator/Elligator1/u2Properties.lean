...
lemma u2_eq_zero
  ...
  u2_of_point = 0 := by
    ...
    rw [z_eq_zero t s_h1 s_h2 q_h1 q_h2 q_h3]
    simp
...
lemma u2_eq_u'
  ...
  u2_of_t = u'_of_t := by
    ...
    have h1 : (c_of_s - 1) * s * X2_of_t * (1 + X2_of_t) = x'_of_t * Y'_of_t := by
      unfold X2_of_t
      rw [X_h]
      rw [← div_left_inj' (Y_ne_zero s_h1 q_h1 q_h2 q_h3 ⟨-t.val, h2_2⟩)]
      change x'_of_t = x'_of_t * Y'_of_t / Y'_of_t
      rw [mul_div_assoc, div_self (Y_ne_zero s_h1 q_h1 q_h2 q_h3 ⟨-t.val, h2_2⟩)]
      ring_nf
...
lemma v'_ne_zero
  ...
  X ≠ 1 → v ≠ 0 := by
    ...
    rw [h2]
    let h3 := Y'_ne_zero s_h1 s_h2 q_h1 q_h2 q_h3 point point_props x_ne_zero y_ne_one
    let z_ne_zero := z'_ne_zero s_h1 s_h2 q_h1 q_h2 q_h3 point point_props x_ne_zero y_ne_one
    grind
...
lemma χ_of_v'_eq_χ_of_z'
  ...
  X ≠ 1 → χ_of_v = χ_of_z := by
    ...
    rw [← LegendreSymbol.χ_of_a_eq_χ_a_mul_b_pow_two h2 q_h1 q_h3]
    grind
...
lemma X'_eq_χ_of_v'_mul_u'
  ...
  X ≠ 1 → X = χ_of_v * u := by
    ...
    let χ_of_v'_eq_z' := χ_of_v'_eq_z' s_h1 s_h2 q_h1 q_h2 q_h3 point point_props x_ne_zero y_ne_one
    unfold χ_of_v v
    rw [χ_of_v'_eq_z' h1]
    let z :=z' s_h2 q_h1 q_h2 q_h3 point
    let z_ne_zero := z'_ne_zero s_h1 s_h2 q_h1 q_h2 q_h3 point point_props x_ne_zero y_ne_one
    ...
    nth_rw 1 [← mul_one X]
    unfold z z'
    rw [mul_div_assoc, LegendreSymbol.one_over_χ_of_a_eq_χ_a q_h1 q_h2 q_h3]
    grind
...
lemma Y'_observation1
  ...
  X ≠ 1 → χ_of_Y = χ_of_v * χ_of_u'_pow_two_add_one_over_c_pow_two  := by
    ...
    have h4 : χ_of_term1 * χ_of_u'_pow_two_add_one_over_c_pow_two = 1 := by
      rw [← pow_two]
      let term1_ne_zero := u'_pow_two_add_one_over_c_pow_two_ne_zero s_h1 s_h2 q_h1 q_h2 q_h3 point
      rw [LegendreSymbol.χ_of_a_even_pow_n_eq_one term1_ne_zero ⟨2, even_two⟩ q_h1 q_h3]
    grind
...
lemma Y'_observation2
  ...
  X ≠ 1 → Y = (χ_of_v * v)^((q + 1) / 4) * χ_of_v * χ_of_u'_pow_two_add_one_over_c_pow_two  := by
    ...
    rw [q_h1, add_comm, LegendreSymbol.a_pow_q_add_one_over_two_eq_χ_of_a_mul_a q_h1 q_h3]
