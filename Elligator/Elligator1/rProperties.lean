...
lemma r_ne_zero (hs_ne_zero : s ≠ 0) (hq_card : Fintype.card F = q) (hq_mod : q % 4 = 3)
  : (r s) ≠ 0 := by
    ...
    have h4 : q % 4 ≠ 3 := by
      rw [FiniteField.isSquare_neg_one_iff, hq_card] at h3
      exact h3
    contradiction
...
lemma four_add_r_ne_zero
  ...
  : 4 + (r s) ≠ 0 := by
    ...
    change 4 + (c + 1 / c) ≠ 0
    intro h
    have hc : c ≠ 0 := c_ne_zero hs_ne_zero hq_card hq_mod
    have h2 : (2 : F) ≠ 0 := FiniteFieldBasic.two_ne_zero hq_card hq_mod
    ...
    have h_not_sq : ¬ IsSquare (-1 : F) := by
      rw [FiniteField.isSquare_neg_one_iff, hq_card]
      omega
    exact h_not_sq h_neg_sq

lemma r_h1 (hs_ne_zero : s ≠ 0) (hq_card : Fintype.card F = q) (hq_mod : q % 4 = 3)
  :
  let r := r s
  let c := c s
  (r^2 - 2) = c^2 + 1 / c^2 := by
...
lemma r_sub_two_ne_zero
  ...
  : (r s) - 2 ≠ 0 := by
    let c := c s
    let c_ne_zero := c_ne_zero hs_ne_zero hq_card hq_mod
    let c_ne_one := c_ne_one sq_ne_pm_two
    change (c + 1 / c) - 2 ≠ 0
