...
@[blueprint "def:χ"
  ...
  (statement := /--
  Fix a prime power $q \equiv 3 \pmod 4$. Define $\chi : \mathbb{F}_q \to \mathbb{F}_q$ as the
  quadratic character of $\mathbb{F}_q$, with its values $0, \pm 1$ read inside $\mathbb{F}_q$;
  equivalently
  $$
  \chi(a) = a^{(q-1)/2} .
  $$
