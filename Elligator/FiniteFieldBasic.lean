...
lemma ringChar_of_F_eq_q (q_prime : Prime (Fintype.card F)) : ringChar F = Fintype.card F := by
...
lemma fin_to_finfield_injective (q_prime : Prime (Fintype.card F)) :
    Function.Injective (fun n : Fin (Fintype.card F) => (n : F)) := by
  intro a b hab
  have h : CharP F (Fintype.card F) := by
    rw [← ringChar_of_F_eq_q q_prime]
    exact ringChar.charP F
  exact Fin.ext (CharP.natCast_injOn_Iio F (Fintype.card F) a.isLt b.isLt hab)

lemma fin_to_finfield_bijective (q_prime : Prime (Fintype.card F)) :
    Function.Bijective (fun n : Fin (Fintype.card F) => (n : F)) :=
  (Fintype.bijective_iff_injective_and_card _).mpr
    ⟨fin_to_finfield_injective q_prime, by rw [Fintype.card_fin]⟩
...
lemma exists_fin_cast_eq (q_prime : Prime (Fintype.card F)) (t : F) :
    ∃ n : Fin (Fintype.card F), (n : F) = t :=
  (fin_to_finfield_bijective q_prime).surjective t
...
lemma exists_nat_cast_eq (q_prime : Prime (Fintype.card F)) (t : F) :
    ∃ (n : ℕ), n < Fintype.card F ∧ (n : F) = t := by
  obtain ⟨n, hn⟩ : ∃ n : Fin (Fintype.card F), (n : F) = t := exists_fin_cast_eq q_prime t
